...
const maxValue =(arr)=>{
    ...
    descendingArr = arr.sort((a,b)=>b-a)
    
    return {
        maxValue:ascendingArr[0],
        newmaxValue:max,
        sortArr:descendingArr
    }
}

console.log( maxValue(arr1) ) //{ maxValue: 63, newmaxValue: 63, sortArr: [ 63, 25, 15, 12 ] }
...
const Store = (product , count) =>{
    let ProductStock = table[product]

    if(!ProductStock) return `${product} (품목없음)`
    if(ProductStock === count) return `전산표와 일치합니다`
    return `전산표와 일치하지 않습니다`
}

console.log( Store("야채곱창",6) ) //전산표와 일치하지 않습니다
...
const PlusFind = (arr , i) =>{
    const resultmap = arr.map( (x)=> x + 10 )
    const resultfilter = resultmap.filter((x)=> x !== i)
    if(resultmap.toString() === resultfilter.toString()) return  `결과값이 없습니다`
    return resultfilter
    
}

console.log( PlusFind(Arr,p) ) //[ 11, 13, 14, 15 ]
...
const Car = (km) =>{
    ...
    const bootStart = "시동이 걸렸습니다"
    const bootStop = "시동이 꺼졌습니다"
    const dirve = "주행합니다"
    const arrive = "목표에 도착했습니다"

    if(km > 40) return `안전상의 이유로 ${bootStop}`
